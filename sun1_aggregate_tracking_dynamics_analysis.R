...
skip_to_next <- FALSE
tryCatch(convHull(xxx), error = function(e) { skip_to_next <<- TRUE})
if(skip_to_next) {  area[n, 1] = NA} else {
...
for (n in 1:(size(nn, 2) / 2)) {
  speed[, n] = ((nn[2:size(nn, 1), m] - nn[1:size(nn, 1) - 1, m]) ^ 2 + (nn[2:size(nn, 1), m +
                                                                              1] - nn[1:size(nn, 1) - 1, m + 1]) ^ 2) ^ 0.5 * 107.8 / 5
...
a[a == 0] = NaN
split_fusion = sum(abs(a[1:size(a, 1) - 1, 1] - a[2:size(a, 1), 1]), na.rm =
                     TRUE)
...
newtime_for_distri=time_for_distri[-1,] #this was used so as to remove the first row. We do not know what the state of the foci were in the previous ones, therefore we must remove it and count with the first non-zero number.
newtime_for_distri[newtime_for_distri==0]<-NA # to convert all the zeroes to NA
